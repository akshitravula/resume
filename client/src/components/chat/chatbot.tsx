...
export function Chatbot({ resumeId, data, setData, wsRef }: ChatbotProps) {
  ...
  // Enhanced WebSocket connection monitoring and message handling
  useEffect(() => {
    const setupWebSocketListeners = () => {
      ...
      const handleClose = (event: CloseEvent) => {
        ...
        setIsTyping(false); // Stop typing indicator when disconnected
      ...
      // Set initial connection status based on current readyState
    ...
    // Monitor for WebSocket reference changes (reconnections)
    const checkConnection = setInterval(() => {
      ...
      if (currentStatus === WebSocket.OPEN && connectionStatus !== "connected") {
        setupWebSocketListeners(); // Re-setup listeners on reconnection
      } else if (currentStatus === WebSocket.CONNECTING && connectionStatus !== "connecting") {
        setConnectionStatus("connecting");
      } else if ((currentStatus === WebSocket.CLOSED || currentStatus === WebSocket.CLOSING) && connectionStatus !== "disconnected") {
        setConnectionStatus("disconnected");
        setIsTyping(false);
      }
    }, 1000);
  ...
  }, [wsRef, setData, connectionStatus]);
  ...
  const sendMessage = (text: string) => {
    ...
    if (!wsRef.current) {
      addSystemMessage("No connection available. Please check your connection and try again.");
      return;
    }

    if (wsRef.current.readyState !== WebSocket.OPEN) {
      addSystemMessage("Connection lost. Please wait for reconnection or refresh the page.");
      return;
    }
  ...
  return (
    <>
      ...
      {!isOpen && (
        <Button
          className="fixed bottom-4 right-4 w-16 h-16 rounded-full shadow-xl z-[9999]"
          onClick={handleOpen}
        >
      ...
      {isOpen && (
        <div 
          ref={chatbotRef}
          className={`fixed z-[9999] ${isDragging ? 'cursor-grabbing' : ''}`}
          style={{
            left: `${position.x}px`,
            top: `${position.y}px`,
              ...
              <div className="p-4 border-t">
                <div className="flex items-center space-x-2">
                  ...
                  <Button 
                    ...
                    size="icon"
                    disabled={(!inputValue.trim() && !interimTranscript.trim()) || connectionStatus === "disconnected"}
                  >
