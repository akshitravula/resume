...
export function JobKeywordMatcher({ resumeData, onSuggestion }: JobKeywordMatcherProps) {
  ...
  const getResumeText = (resume: Resume): string => {
    ...
    resume.work_experiences.forEach(exp => {
      if (exp.designation) parts.push(exp.designation);
      if (exp.designation_description) parts.push(exp.designation_description);
      exp.projects.forEach(proj => {
        if (proj.project_name) parts.push(proj.project_name);
        if (proj.project_description) parts.push(proj.project_description);
        parts.push(...proj.description_bullets);
      });
    });
    
    resume.education_entries.forEach(edu => {
