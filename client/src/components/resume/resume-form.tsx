...
const AIFloatingBar = ({ onSave, onClose, fieldType, position, sectionId }) => {
  ...
  const handleGenerate = async () => {
    ...
    setTimeout(() => {
      const suggestions = {
        summary: "Experienced software developer with 5+ years of expertise in full-stack development, specializing in React, Node.js, and cloud technologies. Proven track record of delivering scalable solutions and leading cross-functional teams.",
        bullet: "Developed and maintained responsive web applications using React and TypeScript, improving user engagement by 40%",
        skill: "JavaScript, TypeScript, React, Node.js, Python"
      };
      setEditedText(suggestions[fieldType] || '');
  ...
  return (
    <div 
      className="absolute z-10 bg-white border rounded-lg shadow-lg p-2 flex flex-col gap-2"
...
export const ResumeForm = React.forwardRef<any, ResumeFormProps>(({ 
  ...
  // AI Bars state - now supports multiple AI bars with section IDs
  const [aiBars, setAiBars] = useState({});

  // Helper function to generate unique section IDs
  const generateSectionId = (section, index = null, subIndex = null) => {
    let id = section;
    if (index !== null) id += `-${index}`;
    if (subIndex !== null) id += `-${subIndex}`;
    return id;
  };

  // Modified openAiBar function
  const openAiBar = (event, fieldType, onSave, sectionId) => {
  ...
  // Modified closeAiBar function
  const closeAiBar = (sectionId) => {
    setAiBars(prev => {
      const newState = { ...prev };
  ...
  // Close all AI bars function (optional)
  const closeAllAiBars = () => {
    setAiBars({});
  };
...
// Internships Updates
const updateInternship = useCallback((index: number, field: keyof Internship, value: string | string[]) => {
  updateFormData(prev => ({
    ...prev,
...
const updateInternshipBullet = useCallback((internshipIndex: number, bulletIndex: number, value: string) => {
  updateFormData(prev => ({
    ...prev,
    internships: [...(prev.internships ?? [])].map((internship, i) => 
      i === internshipIndex ? {
        ...internship,
        internship_work_description_bullets: [...(internship.internship_work_description_bullets ?? [])].map((bullet, j) => j === bulletIndex ? value : bullet)
      } : internship
    )
  }));
}, [updateFormData]);
...
const toggleInternshipVisibility = useCallback((index: number) => {
  updateFormData(prev => ({
    ...prev,
    internships: [...(prev.internships ?? [])].map((internship, i) => 
      i === index ? { ...internship, visible: !internship.visible } : internship
    )
  }));
}, [updateFormData]);
      ...
      <div className="p-6 space-y-8">
        ...
        <div className={`space-y-3 ${!sectionVisibility.summary ? 'opacity-50' : ''}`}>
          ...
          {sectionVisibility.summary && (
            <div className="relative">
              ...
              />
              <button 
                onClick={(e) => openAiBar(e, 'summary', (text) => updatePersonalInfo('summary', text), 'summary-main')}
          ...
          {aiBars['summary-main']?.isOpen && (
            <AIFloatingBar
              onSave={aiBars['summary-main'].onSave}
              onClose={() => closeAiBar('summary-main')}
              fieldType={aiBars['summary-main'].fieldType}
              position={aiBars['summary-main'].position}
                ...
                {(work.projects || []).map((project, projectIndex) => (
                  <div key={projectIndex} className="space-y-2 border-l-2 border-orange-200 pl-4">
                    ...
                    {(project.description_bullets || []).map((bullet, bulletIndex) => {
                      const sectionId = generateSectionId('work-bullet', workIndex, `${projectIndex}-${bulletIndex}`);
                      return (
                        <div key={bulletIndex} className="flex items-center gap-2 ml-6">
                          ...
                          <div className="flex-1 flex items-center gap-2">
                            ...
                            <button 
                              onClick={(e) => openAiBar(
                                e, 
                                'bullet', 
                                (text) => updateWorkProjectBullet(workIndex, projectIndex, bulletIndex, text),
                                sectionId
                              )}
                          ...
                          {aiBars[sectionId]?.isOpen && (
                            <AIFloatingBar
                              onSave={aiBars[sectionId].onSave}
                              onClose={() => closeAiBar(sectionId)}
                              fieldType={aiBars[sectionId].fieldType}
                              position={aiBars[sectionId].position}
                              sectionId={sectionId}
                            />
                          )}
                        </div>
        ...
        <div className={`space-y-4 ${!sectionVisibility.internships ? 'opacity-50' : ''}`} ref={internshipsRef}>
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => toggleSectionVisibility('internships')}
                className="text-gray-400 hover:text-gray-600"
              >
                {sectionVisibility.internships ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
              </button>
            </div>
          ...
          {sectionVisibility.internships && (formData?.internships || []).map((internship, internshipIndex) => (
            <div key={internship.id || internshipIndex} className={`space-y-3 border-l-2 border-gray-200 pl-4 ${!internship.visible ? 'opacity-50' : ''}`}>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => toggleInternshipVisibility(internshipIndex)}
                  className="text-gray-400 hover:text-gray-600"
                >
                  {internship.visible ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                </button>
                {(formData?.internships?.length || 0) > 1 && (
                  <button
              ...
              {(internship.internship_work_description_bullets || []).map((bullet, bulletIndex) => {
                const sectionId = generateSectionId('internship-bullet', internshipIndex, bulletIndex);
                return (
                  <div key={bulletIndex} className="flex items-center gap-2 ml-6">
                    {(internship.internship_work_description_bullets?.length || 0) > 1 && (
                      <button
                    ...
                    <div className="flex-1 flex items-center gap-2">
                      ...
                      <button 
                        onClick={(e) => openAiBar(
                          e, 
                          'bullet', 
                          (text) => updateInternshipBullet(internshipIndex, bulletIndex, text),
                          sectionId
                        )}
                        className="text-orange-500 hover:text-orange-600"
                      >
        ...
        <div className={`space-y-4 ${!sectionVisibility.projects ? 'opacity-50' : ''}`} ref={projectsRef}>
          ...
          {sectionVisibility.projects && (formData.academic_projects ?? []).map((project, projectIndex) => (
            <div key={project.id || projectIndex} className={`space-y-3 border-l-2 border-gray-200 pl-4 ${!project.visible ? 'opacity-50' : ''}`}>
              ...
              {(project.description_bullets ?? []).map((bullet, bulletIndex) => {
                const sectionId = generateSectionId('academic-bullet', projectIndex, bulletIndex);
                return (
                  <div key={bulletIndex} className="flex items-center gap-2 ml-6">
                    ...
                    <div className="flex-1 flex items-center gap-2">
                      ...
                      <button 
                        onClick={(e) => openAiBar(
                          e, 
                          'bullet', 
                          (text) => updateProjectBullet(projectIndex, bulletIndex, text),
                          sectionId
                        )}
                        className="text-orange-500 hover:text-orange-600"
                      >
          ...
          {sectionVisibility.pors && (formData?.positions_of_responsibility || []).map((por, porIndex) => (
            <div key={por.id || porIndex} className={`space-y-3 border-l-2 border-gray-200 pl-4 ${!por.visible ? 'opacity-50' : ''}`}>
              ...
              {(por.responsibilities || []).map((bullet, bulletIndex) => {
                const sectionId = generateSectionId('por-bullet', porIndex, bulletIndex);
                return (
                  <div key={bulletIndex} className="flex items-center gap-2 ml-6">
                    ...
                    <div className="flex-1 flex items-center gap-2">
                      ...
                      <button 
                        onClick={(e) => openAiBar(
                          e, 
                          'bullet', 
                          (text) => updatePORBullet(porIndex, bulletIndex, text),
                          sectionId
                        )}
                        className="text-orange-500 hover:text-orange-600"
                      >
        ...
        <div className={`space-y-4 ${!sectionVisibility.achievements ? 'opacity-50' : ''}`} ref={achievementsRef}>
          ...
          {sectionVisibility.achievements && (formData?.achievements || []).map((achievement, index) => {
            const sectionId = generateSectionId('achievement', index);
            return (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button 
                        onClick={(e) => openAiBar(
                          e, 
                          'bullet', 
                          (text) => updateAchievement(index, 'description', text),
                          sectionId
                        )}
                        className="text-orange-500 hover:text-orange-600"
                      >
        ...
        <div className={`space-y-4 ${!sectionVisibility.skills ? 'opacity-50' : ''}`} ref={skillsRef}>
          ...
          {sectionVisibility.skills && (formData.skills ?? []).map((skill, index) => {
            const sectionId = generateSectionId('skill', index);
            return (
