...
// A4PreviewContainer component
const A4PreviewContainer: React.FC<{
  ...
  return (
    <div className="relative">
      {Array.from({ length: pageCount }).map((_, index) => (
        <div
          key={index}
          className={`bg-white shadow-lg mx-auto mb-6 relative ${
            index > 0 ? 'mt-8' : ''
          }`}
          style={{
        ...
        >
          {index === 0 ? (
            ...
            <div className="p-8">
              <div className="text-center text-gray-500 py-4 border-b">
                <p>Page {index + 1}</p>
                <p className="text-sm">(Resume continuation)</p>
              </div>
            </div>
          )}

          {/* Divider line at bottom of each page except last */}
          {index < pageCount - 1 && (
